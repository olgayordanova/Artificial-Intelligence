...
def read_data_from_file(data_file):
    ...
    candles_df = pd.DataFrame(df, columns=['T', 'open', 'high', 'low', 'close', 'volume', 'CT', 'QV', 'N', 'TB', 'TQ', 'I'])
...
def make_chart(candles_df):
    try:
        ...
        adps = [mpf.make_addplot(candles_df['EWM'][CHART_MIN:CHART_MAX], color='r', linestyle='dotted'),
                mpf.make_addplot ( candles_df['RSI'][CHART_MIN:CHART_MAX], panel=1, color='black',linestyle='dotted', secondary_y=True ),
                mpf.make_addplot(td_bull_esc[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='^', color='green'),
                mpf.make_addplot(td_bear_esc[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='v', color='red'),
                mpf.make_addplot(td_inv_ham[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='^', color='grey'),
                mpf.make_addplot(td_ham[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='^', color='orange'),
                mpf.make_addplot(td_mor_star[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='^', color='cyan'),
                mpf.make_addplot(td_eve_star[CHART_MIN:CHART_MAX], type='scatter', markersize=MARKERSIZE, marker='v', color='magenta'),
                ]
        mpf.plot(candles_df[CHART_MIN:CHART_MAX], type='candle', style='yahoo', volume=True, volume_panel=1, figratio=(15, 7), addplot=adps,
                 title=f"\n\n{company_name} prices,\n six popular signals", savefig=f"pictures/{company_name}.png") #, savefig=f"pictures/{company_name}.png"
    except:
        return

def plot_hit_ratio(df_coefficients):
    #TODO How STOP_LEVEL impact to hit_ratio_mean - make chart for different stop-levels - market philology
    mean_hit_ratios = {}
    combined_label = ''
